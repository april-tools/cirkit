...
from cirkit.layers.input.exp_family import ExpFamilyLayer, NormalLayer, CategoricalLayer, BinomialLayer 
...
class SymbolicSumNode(SymbolicNode):  
    ...
    def set_placeholder_params(self, 
                   num_input_units: int, 
                   num_output_units: int, 
                   reparam: ReparamFactory = ReparamIdentity, ) -> None:  
        ...
        if self.layer_cls == TuckerLayer: 
            self.params = reparam((1, num_input_units, num_input_units, num_output_units), dim=(1,2))
