from typing import TYPE_CHECKING, Callable, Dict, Tuple, cast
...
class DenseCompositionPattern(CircuitOptPatternDefn):
    entries = {
        0: LayerOptEntry(cls=TorchDenseLayer),
        1: LayerOptEntry(cls=TorchDenseLayer),
    }
...
class TuckerPattern(CircuitOptPatternDefn):
    entries = {
        0: LayerOptEntry(cls=TorchDenseLayer),
        1: LayerOptEntry(cls=TorchKroneckerLayer),
    }
...
class CandecompPattern(CircuitOptPatternDefn):
    entries = {
        0: LayerOptEntry(cls=TorchDenseLayer),
        1: LayerOptEntry(cls=TorchHadamardLayer),
    }


class DenseKroneckerPattern(LayerOptPatternDefn):
    entry = LayerOptEntry(cls=TorchDenseLayer)
    patterns = {
        "weight": ParameterOptPattern(
            entries={0: ParameterOptEntry(cls=TorchKroneckerParameter)}, output=True
        )
    }
