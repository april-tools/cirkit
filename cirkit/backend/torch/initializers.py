...
def mixing_weights_(
    ...
    if len(tensor.shape) != 2 or tensor.shape[1] % tensor.shape[0] != 0:
        raise ValueError(
            f"Expected destination shape (num_units, arity * num_units), but found {tensor.shape}"
        )
    num_units = tensor.shape[0]
    arity = tensor.shape[1] // tensor.shape[0]
