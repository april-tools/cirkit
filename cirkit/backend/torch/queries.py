...
class SamplingQuery(Query):
    ...
    def __call__(self, num_samples: int = 1) -> tuple[Tensor, list[Tensor]]:
        ...
        @return: A tuple t with t[0] the samples generated by the computational graph, and
            t[1] the list of samples of the mixture branches
    ...
    def _pad_samples(self, samples: Tensor, scope_idx: Tensor) -> Tensor:
        ...
        @param samples: The samples to pad
        @shape y: (folds, channels, cardinality, samples)
        @param scope_idx: The scope index of the input layer outputting the samples,
            a tensor of shape (folds, num_variables), where num_variables is the size of
            the scope of each layer in the folded layer.
