...
def check_smoothness_decomposability(rg: RegionGraph):
    for r in rg.inner_region_nodes:
        iscopes: Set[Tuple] = set(map(lambda n: tuple(n.scope), rg.get_node_input(r)))
    ...
    for p in rg.partition_nodes:
        iscopes: List[Set] = list(map(lambda n: n.scope, rg.get_node_input(p)))
        for i, j in itertools.combinations(range(len(iscopes)), 2):
            assert len(iscopes[i] & iscopes[j]) == 0, "Decomposability is not satisfied"
...
def check_strong_structured_decomposability(rg: RegionGraph):
    ...
    for p in rg.partition_nodes:
        ...
        if scope not in decomps:
            ...
            continue
        oth_iscopes = set(map(tuple, iscopes))
        cur_iscopes = set(map(tuple, decomps[scope]))
...
def check_region_partition_layers(rg: RegionGraph, bottom_up: bool):
    ...
    for i, layer in enumerate(layers):
        if i % 2 == 0:  # Layer of regions at the inputs
            for r in layer:
...
@pytest.mark.parametrize(
    "num_vars,depth,num_repetitions",
    list(itertools.product([1, 8, 16], [0, 1, 3], [1, 3])))
...
@pytest.mark.parametrize(
    "size,struct_decomp",
    list(itertools.product([(1, 1), (17, 17), (32, 32)], [False, True])))
...
def test_rg_poon_domingos(shape: Tuple[int, int], delta: Union[float, List[float], List[List[float]]]):
