...
def test_overparameterized_sdd(fold, optimize, input_layer, num_units):
    ...
    s_c = sdd_c.build_circuit(
        enforce_smoothness=False, input_layer=input_layer, num_units=num_units
    )
    t_c = ctx.compile(s_c)
...
def test_conditional_sdd_circuit(fold, optimize, input_layer):
    sdd_c = SDD.from_string(SDD_s)
    s_c = sdd_c.build_circuit(input_layer=input_layer, sum_weight_activation="softmax")
    cond_s_c, gf_specs = SF.condition_circuit(s_c, gate_functions={"sum": list(s_c.sum_layers)})
